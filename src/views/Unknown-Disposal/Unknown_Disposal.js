...
import {  getExportFile, getUnknownDisposaldata } from '../../redux/UnknownDisposal'
...
const  Unknown_Disposal = ({t}) => {
    ...
    {
        accessor:"SludgeCollected",
        filter: "false",
        header: "Sludge Collected (IN KL)",
        sortable: "false"
    },
    {
        accessor:"Comments",
    ...
    const handleFilter = e => {
        ...
        dispatch(getUnknownDisposaldata(payload))
    }
    ...
    const fstpReportsStore = store.UnknownDisposal.UnknownDisposalData ? store.UnknownDisposal.UnknownDisposalData.data ? store.UnknownDisposal.UnknownDisposalData.data.data : [] : []
    const lengthUlb = store.UnknownDisposal.UnknownDisposalData ? store.UnknownDisposal.UnknownDisposalData.data ? store.UnknownDisposal.UnknownDisposalData.data.totalCount : 0 : 0
    ...
    return (
        <div>
            <Fragment>
                <Breadcrumbs title="Unknown Disposal" data={[{ title: 'UnknownDisposal' }, { title: `Unknown Disposal` }]} />
            </Fragment>
                    ...
                    <Row className="align-items-center">
                        ...
                        <InputGroup className=''>
                            ...
                            <Input
                                ...
                                type='text'
                                bsSize='sm'
                                placeholder={searchSelect === "selected" ? `${t('searchby')}` : searchSelect === "operatorname" ? `Search by Operator name` : searchSelect === "truckNo" ? `Search by Truck No.` : ""}
                                id='search-input'
                                    ...
                                    columns.map((accesser, h) => {
                                        switch (accesser.accessor) {
                                            case "checkbox":
                                                ...
                                                /></div>
                                            case "CreatedAt":
                                                return val["CreatedAt"] === null ? "_" : <td key={h}>{moment(val["CreatedAt"]).format("lll")}</td>
                                            case "Photo":
                                                return val["Photo"] === null || val["Photo"] === "null" ? <td>_</td> : <td><a href={`${val["Photo"]}`}>{t('View')}</a></td>
                                            default:
...
export default (withTranslation()(Unknown_Disposal))
